...
func (c TrackerController) Index(w http.ResponseWriter, r *http.Request) {
	...
	http_utils.RespondWithJSON(w, http.StatusOK, TrackerSerializer{}.SerializeTrackers(*trackers))
}
...
func (c TrackerController) Show(w http.ResponseWriter, r *http.Request) {
	...
	http_utils.RespondWithJSON(w, http.StatusOK, TrackerSerializer{}.SerializeTracker(*tracker))
}

func (c TrackerController) Create(w http.ResponseWriter, r *http.Request) {
	var tracker domain.TrackerModel
	json.NewDecoder(r.Body).Decode(&tracker)
	c.TrackerService.Create(&tracker)
	http_utils.RespondWithJSON(w, http.StatusCreated, TrackerSerializer{}.SerializeTracker(tracker))
}
